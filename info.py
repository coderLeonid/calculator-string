...
text_help_calc = (" строка-калькулятор | быстрый ввод • мгновенное вычисление • точный ответ\n\n"
                  ...
                  " ● Если вставить текст из истории или других источников, он форматируется под строку или другое окно на уровне операторов (не функций)\n"
                  "\nРедактирование поля ввода (условимся, что 'I' — это курсор, или каретка)\n"
                  " ⬥ При стирании '•√I' после нажатия [r] останется '√I'\n"
                  " ⬥ '/100' [P] (percent — процент, 1/100 часть), '•10^' [E], (2e5 — компактная 2•10^5), [;] пишет '(1/I)' и [.] пишет '0.', если перед курсором не цифра\n"
                  ...
                  " ⬥ при [цифра] после '0' введётся '.' с цифрой, при [z, M, B, T, Q] после точки введётся на одну меньше нулей '0', после нуля тоже самое с точкой впереди\n"
                  " ⬥ [v] вводит длиннейшую повторяющуюся последовательность без чисел или констант на концах, [V] — 2-ю по количеству символов последовательность\n"
                  " ⬥ '•' ставится автоматически. При курсоре после π, 1, при скобках после '!' подставится '/' (удобно вводить радианы 'k•π/m' и сочетание 'n!/(k!•(n-k)!)')\n"
                  " ⬥ Умножить не ставится при [|], только с [a], так как пример можно понять по-разному ('|5+5|7+8|2+2|', как '|5+5|•7+8•|2+2|' или '|5+5•|7+8|•2+2|')\n"
                  " ⬥ После любого знака или в начале строки [•] введёт '2•', [/] — '1/' (кроме случаев: после '•' — '/2', после '√' — '/3')\n"
                  " ⬥ если [+] после '•', '/', '^', '√', то он будет вводиться с '2', иначе заменится на '1'; если после '.' поставить любой знак, кроме [.], появится '5'\n"
                  ...
                  " ★ Автоматичское добавление необходимых знаков при вводе примера\n"
                  ...
                  " ★ Открой строку за 15 секунд после случайного закрытия и пример останется. Иначе перейдёт в историю, если в ответе не число — удалится\n"
                  "\nНастройки и вид\n"
                  " ⚙ Размещение. Перетаскивай калькулятор мышкой, комбинациями ctrl+shift+↑↓ или написанием y# либо -y#, где y — отступ сверху, -y — снизу\n"
                  " ⚙ Низ экрана: '1-#' или '-#', при 'y' от 0 до 1 не включительно, это число высот экрана, а не пикселей, 'y' можно писать примерами ('4^2/7#')\n"
                  " ⚙ Округление. По умолчанию: пиши где угодно в примере q, до нужного знака: qA (A от -23 до 17) и оно автоматически применится за секунду\n"
